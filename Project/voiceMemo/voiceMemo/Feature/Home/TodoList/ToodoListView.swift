...
struct TodoListView: View {
    ...
    // StateObject로도 가능하지만, 다른 뷰에서도 todolistViewModel이 사용되므로 environment로도 고려 가능
    
    var body: some View {
        ZStack {
            // 투두 셀 리스트
            VStack {
                // 투두가 있는 경우에만 오른쪽 버튼 생성
...
private struct AnnouncementView: View {
    fileprivate var body: some View {
        VStack(spacing: 15) {
            ...
            Image("pencil")
                .renderingMode(.template) // 색상을 텍스트와 같이 변화시키기 위해
            Text("\"매일 아침 5시 운동하자!\"")
            Text("\"내일 8시 수강 신청하자!\"")
...
private struct TodoCellView: View {
    ...
    fileprivate init(
        ...
        _isRemoveSelected = State(initialValue: isRemoveSelected) // State로 초기화 시킬땐 _변수명
        self.todo = todo
    }
